...
public class UserController {
	...
	public ModelAndView editedAuthor(@Valid User user, BindingResult result) {
		ModelAndView mav = new ModelAndView();
		if (!result.hasErrors()) {
			userRepository.saveAndFlush(user);
			mav.setViewName("redirect:http://localhost:8080/Application_Twitter/confirmation");
			return mav;
		} else {
			mav.setViewName("form/editUser");
			return mav;
	...
	public ModelAndView removeArticle(@PathVariable long id) {
		ModelAndView mav = new ModelAndView();
